...
export const ColorVariant = ({ opt }: IProps) => {
  ...
  const content = (
    <>
      <div
        className={`absolute top-1/2 left-1/2 h-9 w-9 -translate-x-1/2 -translate-y-1/2 transform rounded-full ring-2 transition-colors hover:ring-border-soft-hover ${opt.isCurrent ? "ring-primary-solid hover:ring-primary-solid" : "ring-transparent"}`}
      />
      {isDisabled && (
